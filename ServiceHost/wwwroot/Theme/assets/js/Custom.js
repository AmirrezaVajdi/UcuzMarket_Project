...
async function GetPrductsId() {
    ...
    if (storage === null) {
        ...
        var pM = JSON.parse(storage);
    }

    pM.forEach(function (product) {
        productsId.push(product.Id);
    })
    var json = JSON.stringify(productsId);

    var headersList = {
        "Content-Type": "application/json"
    }
    var response = await fetch(fullUrl + "/api/product/GetProductsCheckout", {
        method: "POST",
        body: json,
        headers: headersList
    });
...
">
                            ...
                            <td>${ToPersianNumber(pM.find(function (p) { return p.Id == product.id }).count)}</td>
                            <td class="h6 py-3 d-none d-md-table-cell">
                            ${(product.priceWithDiscount != null ? ToPersianNumber((ToEnglishNumber(product.priceWithDiscount) * ToEnglishNumber(pM.find(function (p) { return p.Id == product.id }).count))) + ' تومان' : ToPersianNumber(ToEnglishNumber((product.price == null ? 0 : product.price)) * ToEnglishNumber(pM.find(function (p) { return p.Id == product.id }).count)) + ' تومان')
            }
                            </td >
                        </tr >
    `
        ...
        let pc = ToEnglishNumber((product.price == null ? 0 : product.price));
        let pwd = ToEnglishNumber((product.priceWithDiscount != null ? product.priceWithDiscount : 0));
        let count = pM.find(function (p) { return p.Id == product.id }).count;
    ...
    document.getElementById("totalProductCount").innerText = 'مجموع قیمت' + '(' + ToPersianNumber(pM.length) + ' محصول' + ')';
